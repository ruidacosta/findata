...
class Database(object):
    ...
    def init(self, db_name: str):
        ...
        self.rates_table = db.Table('rates', self.metadata, 
                db.Column('rate_type', db.String(50), primary_key=True),
                db.Column('rate_date', db.String(50), primary_key=True),
                db.Column('rate_value', db.Float, nullable=False))
        self.metadata.create_all(self.engine, checkfirst=True)

    def save(self, type: str, date: str, value: float):
        query = db.insert(self.rates_table)
        values = [{ 'rate_type': type, 'rate_date': date, 'rate_value': value }]
        try:
...
def sqlite_store(db_name: str, type: str, date: str, value: float) -> None:
...
def control_storage(config: dict, date: str, value: float) -> None:
    try:
        ...
        if type == "stdin":
            ...
            type = config["Source"]["rate_type"]
            sqlite_store(db_name, type, str(date), value)
    except:
...
def proccess_values(date: str, value: str):
    date_value = datetime.strptime(date, "%B %d %Y").strftime("%Y-%m-%d")
